...
function hdrs(token?: string) {
  const h: Record<string,string> = {'Content-Type':'application/json','X-API-Key':cfg().apiKey}
...
export async function registerUser(
  username: string, pw: string, mc: string
): Promise<{success:boolean;error?:string}> {
  if (username.length < 3 || username.length > 20) return {success:false,error:'Username: 3–20 characters.'}
  if (!/^[a-zA-Z0-9_]+$/.test(username)) return {success:false,error:'Username: letters/numbers/underscore only.'}
  if (pw.length < 8) return {success:false,error:'Password: min. 8 characters.'}
  if (!/[A-Z]/.test(pw)) return {success:false,error:'Password needs an uppercase letter.'}
  if (!/[0-9]/.test(pw)) return {success:false,error:'Password needs a number.'}

  const salt = generateSalt()
  const passwordHash = await hashPassword(pw, salt)

  const url = baseUrl()
  if (!url) return {success:false,error:'Backend not configured. Go to Settings and add your backend URL.'}
...
// ── Login ─────────────────────────────────────────────────
export async function loginUser(username: string, pw: string): Promise<{success:boolean;user?:User;error?:string}> {
  const url = baseUrl()
  if (!url) return {success:false,error:'Backend not configured. Go to Settings first.'}
  ...
  try {
    ...
    if (!saltR.ok) return {success:false,error:'Wrong username or password.'}
    ...
    const d = await r.json()
    if (!r.ok) return {success:false,error:d.error||'Wrong username or password.'}
...
export function getSessionToken(): string|null {
  try {
    ...
    if (!s||s.expiresAt<Date.now()) { localStorage.removeItem(SESSION_KEY); return null }
    return s.token
  } catch { return null }
}

export async function getCurrentUser(): Promise<User|null> {
  const token = getSessionToken(); if (!token) return null
  const url = baseUrl(); if (!url) return null
  try {
...
export async function updateBalance(userId: string, balance: number, totalWon: number, totalLost: number, gamesPlayed: number) {
...
export async function pushLiveEvent(username: string, game: string, amount: number, won: boolean, multiplier?: number) {
  const url = baseUrl(); if (!url) return
  try {
